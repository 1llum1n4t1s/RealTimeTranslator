...
public partial class App : Application
{
    ...
    private void ConfigureServices(IServiceCollection services)
    {
        ...
        // プロバイダ各実装を具象型で登録する。 TranslationPipelineService が全部を ctor 注入し、
        // AppSettings.Provider に応じて hot-swap する (再起動不要、 次の Start で反映)。
        // DebugAudioRecorder は WebSocket 系 client に ctor 注入される (録音は active 側でのみ発火)。
        services.AddSingleton<OpenAIRealtimeClient>();
        services.AddSingleton<GeminiLiveClient>();
        services.AddSingleton<SonioxRealtimeClient>();
        services.AddSingleton<SpeechmaticsRealtimeClient>();
        services.AddSingleton<AzureSpeechTranslationClient>();
