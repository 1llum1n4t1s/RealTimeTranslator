...
- `UpdateService.cs` の `GithubSource(..., accessToken: string.Empty, prerelease: false)` で
  ユーザー側でバージョン指定はできない。
- `.github/workflows/release.yml` の Cleanup ジョブは「latest 10 件以外を削除」(rere 修正後)。
  これにより**直近の壊れていない既知バージョンの nupkg を保持** している。
...
- ユーザーは PeriodicTimer が走れないため、 forward-fix の v1.0.(X+1) も自動取得できない。
- **手動 Setup.exe 再インストール** が唯一の解。
- §1-A で latest を v1.0.(X-1) に戻していれば、 ユーザーは v1.0.(X-1) を Setup.exe で入れ直して
  以降の更新サイクルに復帰できる。
...
- `.github/workflows/release.yml` (cleanup latest 10 件保持の設定)
