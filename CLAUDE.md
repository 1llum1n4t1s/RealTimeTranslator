...
- **設定**: `AppSettings.AudioCapture` に `EnableVad=true` / `VadPreset="Balanced"` / `VadThreshold=0.4` / `VadPreRollMs=800` / `VadHangoverMs=600` / `AutoPauseOnSilenceSec=0` (v1.0.31 現在)。 UI は「音声処理」タブ (`MainWindow.axaml` Tab 3)。
- **VAD プリセット (`VadPreset`)** (v1.0.31 現在): `Balanced` (推奨 threshold=0.4 / preroll=800 / hangover=600) / `PrioritizeEdges` (頭尻尾重視 threshold=0.3 / preroll=1000 / hangover=800) / `AggressiveSavings` (節約重視 threshold=0.5 / preroll=500 / hangover=350) / `Custom` (詳細 3 値を個別調整) の 4 値。 Custom 以外を選択すると `SettingsViewModel.SelectedVadPreset` setter + `SanitizeSettings` の両方が Threshold/PreRoll/Hangover を **強制同期** する。 settings.json を手で書いて preset 名は Balanced のまま 3 値だけ別値、 は次回起動時に上書きされる仕様。 v1.0.30 で threshold を 0.5 → 0.3 にシフトしたが、 server VAD が句点を返さなくなる回帰が発生したため v1.0.31 で 0.4 に戻り気味調整 (全プリセット +0.1 連動)。 遠距離小音量の声拾いは入力プリプロセス DSP に役割分担。
