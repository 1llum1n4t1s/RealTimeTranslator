...
| `TranslationPipelineService.SentenceSplit.test.cs` | 句点分割 + transcript.done 累積差分抽出 |
...
4. Delta events fire `SubtitleGenerated` with `IsFinal=false` (throttled per `TranslationPipelineService.DeltaThrottle`, 現在 30ms)、 句点 (`。！？.!?`) 到達または D-7 fallback (`MaxPartialChars`、 default 80) 発火で完結文を切り出して `IsFinal=true` 発火 + 新 `SegmentId` 発行
...
- 確定字幕 (`IsFinal=true`) を TSV 形式で永続化: `%APPDATA%/Roaming/RealTimeTranslator/logs/translations/TranslationLog_yyyyMMdd.tsv` (Roaming AppData なので Velopack 自動更新で消えない)。
...
| # | Tab | アイコン | 主な内容 |
|---|---|---|---|
| 1 | メイン | home | プロセス選択 / 出力言語 / ステータス / 開始停止ボタン |
| 2 | API設定 | key | APIキー入力 / 接続テスト / **OpenAI APIキー取得方法ガイド (5 ステップ + 課金目安 + ダッシュボードリンク)** |
| 3 | 音声処理 | waveform | VAD プリセット / Custom 詳細スライダー / 自動 Pause |
...
- **「更新チェックは無効です」が表示される**: `UpdateService.cs:61` で `!_settings.Enabled || string.IsNullOrWhiteSpace(_settings.UpdateBaseUrl)` を判定して `UpdateStatus.Disabled` を発火する。 `UpdateBaseUrl` は `[JsonIgnore]` ハードコード固定 (`https://rtt.nephilim.jp`) で常に非空のため、 **実質的に唯一の発火経路は `settings.json` の `Update.Enabled = false`**。 デフォルトは `true` で UI トグルもないため、 過去にユーザーが settings.json を手動編集したか、 ファイル破損が原因。 対処は `%APPDATA%/RealTimeTranslator/settings.json` を開いて `"Update": { "Enabled": false }` を `true` に書き換えてアプリ再起動。 もしくは settings.json 削除でデフォルト再生成 (ただし API キー等も消える)。
...
| `velopack.yml` | `workflow_call` | Velopack pack ワークフロー (vpk バージョン固定 `0.0.1298` で実行) |
